...
function renderAttachmentFiles(files){
  ...
  if(attachmentList){
    ...
    selectedFiles.forEach((file, index) => {
      ...
      item.innerHTML = `<div class="attachment-item-left"><button class="attachment-remove" data-index="${index}" type="button" aria-label="Remove file">×</button><span>${file.name}</span></div><div class="attachment-item-right"><small>${Math.round(file.size / 1024)} KB</small><span class="attachment-checkmark">✓</span></div>`;
    ...
    attachmentList.querySelectorAll('.attachment-remove').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        renderAttachmentFiles(Array.from(attachmentInput.files));
      });
    });
  }

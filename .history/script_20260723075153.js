...
function renderTimeSlots(iso){
  ...
  // TEST FLAG: set to true to force-disable 18:00 on today's date for demonstration
  const FORCE_TEST_DISABLE_18 = true;
  const todayIso = formatLocalIsoDate(new Date());
  slots.forEach(s=>{
    ...
    try{
      ...
      // Also allow a forced test-disable for 18:00 today
      if(diff < THRESHOLD_MS || (FORCE_TEST_DISABLE_18 && iso === todayIso && s === '18:00')){
        btn.disabled = true;

...
// Update control sizing CSS variables based on measured control heights.
function updateControlVariables(){
  try{
    // Prefer measuring the date input when present so other controls match its height on small screens
    const refInput = document.querySelector('.intake-form input[type="date"]') || document.querySelector('.intake-form input');
    const refTextarea = document.querySelector('.intake-form textarea');
    if(refInput){
      const rect = refInput.getBoundingClientRect();
      const h = Math.round(rect.height) + 'px';
      document.documentElement.style.setProperty('--control-min-height', h);
      const cs = window.getComputedStyle(refInput);
      const bw = cs.borderTopWidth || cs.borderWidth || '5px';
      document.documentElement.style.setProperty('--control-border-width', bw);
    }
    if(refTextarea){
      const rect2 = refTextarea.getBoundingClientRect();
      document.documentElement.style.setProperty('--textarea-min-height', Math.round(rect2.height) + 'px');
    }
  }catch(e){ console.warn('updateControlVariables failed', e) }
}

function debounce(fn, wait){ let t; return (...args)=>{ clearTimeout(t); t = setTimeout(()=>fn(...args), wait); }; }

window.addEventListener('load', ()=>{ updateControlVariables(); setTimeout(updateControlVariables, 300); });
window.addEventListener('resize', debounce(updateControlVariables, 120));

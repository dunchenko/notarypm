...
function renderAttachmentFiles(files){
  ...
  if(attachmentList){
    ...
    selectedFiles.forEach((file, index) => {
      ...
      item.innerHTML = `<span>${file.name}</span><div class="attachment-item-right"><small>${Math.round(file.size / 1024)} KB</small><button class="attachment-remove" data-index="${index}" type="button" aria-label="Remove file">×</button></div>`;

...
function renderAttachmentFiles(files){
  ...
  if(attachmentStatus){
    attachmentStatus.textContent = selectedFiles.length
      ? `${selectedFiles.length} file${selectedFiles.length > 1 ? 's' : ''} ready for future Google Drive upload`
      : 'Ready for future Drive integration';
  }
  if(attachmentList){
    attachmentList.innerHTML = '';
    selectedFiles.forEach(file => {
      const item = document.createElement('li');
      item.innerHTML = `<span>${file.name}</span><small>${Math.round(file.size / 1024)} KB</small>`;
